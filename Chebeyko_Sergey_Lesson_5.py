...
# 2. Есть пустой список. Записать в этот список все числа в диапазоне от 1 до 500 кратные 5. Вывести на печать этот список
# 3. Вывести на экран все чётные значения в диапазоне от 1 до 497, разделяя их запятой
# 4. Дан список чисел (задаете сами). Если число встречается более двух раз, то добавить его в новый список. Если число уже есть в новом списке, то не добавляем его.
# 5. Дополнительная задача. Оформить таблицу умножения: поменять цвет текста, фона и добавить сетку в таблицу.
#Файл называем Chebeyko_Sergey_Lesson_5.py - все 5 задач в одном файле. Сдать до понедельника!
...
print("\033[4m\033[37m\033[46m" 'Таблица умножения вариант 2')
for i in range(1, 10):
    for j in range(1, 10):
        print("%4d" % (i * j), end="\t"'|')
    print()

print("\033[4m\033[35m\033[40m"'Таблица умножения вариант 3')
